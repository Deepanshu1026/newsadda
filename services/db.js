...
async function fetchDatabaseRaw() {
  ...
  // 2. Try reading from the serverless writable temp directory fallback
  try {
...
export async function writeDatabase(posts) {
  // 1. Write to Vercel KV if connected
  if (kvUrl && kvToken) {
    try {
  ...
  // 2. Write to /tmp/database.json for local/container consistency
  try {
  ...
  // 3. Attempt to write to local database.json (works in local dev, fails gracefully on Vercel without throwing)
  try {
